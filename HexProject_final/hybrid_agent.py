...
def a_star_distance(game, player):
    ...
    while pq:
        ...
        if f_val - heuristic(r,c) > current_dist: continue
        ...
        for dr, dc in directions:
            ...
            if 0 <= nr < size and 0 <= nc < size:
                ...
                move_cost = 0 if cell == player else 1
                
                if current_dist + move_cost < dist[nr, nc]:
                    dist[nr, nc] = current_dist + move_cost
                    priority = dist[nr, nc] + heuristic(nr, nc)
...
def get_visual_path(game, player):
    ...
    if player == PLAYER_1:
        for c in range(size):
            if game.board[0, c] != -player:
                start_node = (0, c)
                cost = 0 if game.board[0, c] == player else 1
                pq.append((cost + heuristic(0, c), start_node))
                came_from[start_node] = None
                cost_so_far[start_node] = cost
    else:
        for r in range(size):
            if game.board[r, 0] != -player:
                start_node = (r, 0)
                cost = 0 if game.board[r, 0] == player else 1
                pq.append((cost + heuristic(r, 0), start_node))
                came_from[start_node] = None
                cost_so_far[start_node] = cost
    ...
    while pq:
        ...
        if (player == PLAYER_1 and r == size - 1) or (player == PLAYER_2 and c == size - 1):
            if cost_so_far[current] < min_final_cost:
                min_final_cost = cost_so_far[current]
                final_node = current
            break
        ...
        for dr, dc in directions:
            ...
            if 0 <= nr < size and 0 <= nc < size:
                ...
                new_cost = cost_so_far[current] + (0 if cell == player else 1)
                next_node = (nr, nc)
...
class HybridAI:
    ...
    def check_immediate_threats(self, game):
        ...
        # 1. KAZANMA HAMLESİ (Her modda %100 görmeli)
        ...
        # 2. ZORUNLU BLOK (Rakip kazanmak üzereyse)
        for move in valid_moves:
            game.board[move[0], move[1]] = opp
            if game.check_win(opp):
                game.board[move[0], move[1]] = 0
                
                # --- EASY MODU GÜNCELLEMESİ ---
                if self.difficulty == "Easy":
                    if random.random() < 0.60: 
                        return move, "PANIC_BLOCK" # Bloklasın
                    else:
                        pass 
                else:
                    # Medium ve Hard her zaman bloklar
    ...
    def get_best_move(self, game):
        ...
        if reflex_move:
            return reflex_move, {
                ...
                "log": f"AI Reflex [{self.difficulty}]: {reason}" 
            }
        ...
        return best_move, {
            ...
            "log": "AI MCTS Strategy"
        }

def update_rl_weights(game_history, winner):
    global RL_WEIGHTS
    learning_rate, gamma = 0.02, 0.95
    log_output = f"Updating RL weights based on {len(game_history)} moves...\n"
    G = 1.0
    ...
    try:
        ...
        log_output += f"Weights saved: {np.round(RL_WEIGHTS, 2)}"
    except Exception as e: 
        log_output += f"Error saving weights: {e}"
